...
SpecData = FloatArray | tuple[FloatArray, FloatArray]
DatasetItem = tuple[torch.Tensor, FloatArray, tuple[int, IntArray]]
MetricsDict = dict[str, float]
AudioInput = str | Path | bytes


def _load_preprocess_file(
    audio: AudioInput,
    *,
    seg_length: int | None,
    max_length: int | None,
    ...
    spec = get_librosa_melspec(
        ...
        sr=ms_sr,
        n_fft=ms_n_fft,
        hop_length=ms_hop_length,
        win_length=ms_win_length,
        n_mels=ms_n_mels,
        fmax=ms_fmax,
    ...
    if seg_length is not None:
        ...
        if max_length is not None:
            x_padded = np.zeros((x_spec_seg.shape[0], max_length))
    ...
    return x_spec_seg.unsqueeze(0), torch.tensor([int(n_wins)], dtype=torch.long)
    ...
    def __init__(
        ...
        if cnn_model == "adapt":
            ...
            assert (kernel_size == 3) or (kernel_size == (3, 3)), (
                "cnn_kernel_size is {} should be 3, use adaptive model or change cnn_kernel_size".format(kernel_size)
            )
...
class StandardCNN(nn.Module):
    ...
    def __init__(self, input_channels, c_out_1, c_out_2, c_out_3, kernel_size, dropout, fc_out_h=None):
        super().__init__()
        ...
        self.pool_first = nn.MaxPool2d(self.pool_size, stride=self.pool_size, padding=(0, 1))

        self.pool = nn.MaxPool2d(self.pool_size, stride=self.pool_size, padding=0)

        self.conv1 = nn.Conv2d(self.input_channels, self.c_out_1, self.kernel_size, padding=1)

        self.bn1 = nn.BatchNorm2d(self.conv1.out_channels)

        self.conv2 = nn.Conv2d(self.conv1.out_channels, self.c_out_2, self.kernel_size, padding=1)

        self.bn2 = nn.BatchNorm2d(self.conv2.out_channels)

        self.conv3 = nn.Conv2d(self.conv2.out_channels, self.c_out_3, self.kernel_size, padding=1)

        self.bn3 = nn.BatchNorm2d(self.conv3.out_channels)

        self.conv4 = nn.Conv2d(self.conv3.out_channels, self.c_out_3, self.kernel_size, padding=1)

        self.bn4 = nn.BatchNorm2d(self.conv4.out_channels)

        self.conv5 = nn.Conv2d(self.conv4.out_channels, self.c_out_3, self.kernel_size, padding=1)

        self.bn5 = nn.BatchNorm2d(self.conv5.out_channels)

        self.conv6 = nn.Conv2d(self.conv5.out_channels, self.c_out_3, self.kernel_size, padding=1)
...
class LSTM(nn.Module):
    ...
    def __init__(self, input_size, lstm_h=128, num_layers=1, dropout=0.1, bidirectional=True):
        super().__init__()
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, src, n_wins=None):
        ...
        if n_wins is not None:
            mask = ~(
                (torch.arange(src.shape[0])[None, :]).to(src.device) < n_wins[:, None].to(torch.long).to(src.device)
            )
        else:
            mask = None
...
class PoolLastStepBi(nn.Module):
    ...
    def forward(self, x, n_wins=None):
        ...
        x = torch.cat((x[torch.arange(x.shape[0]), n_wins.type(torch.long) - 1, 0, :], x[:, 0, 1, :]), dim=1)
        x = self.linear(x)
        return x
...
def predict_mos(
    ...
    with torch.no_grad():
        y_hat_list = [
            [model(xb.to(dev), n_wins.to(dev)).cpu().numpy(), yb.cpu().numpy()] for xb, yb, (idx, n_wins) in dl
        ]
...
def predict_mos_file(
    ...
    *,
    ...
    xb, n_wins = _load_preprocess_file(
        audio,
        seg_length=seg_length,
        max_length=max_length,
        seg_hop_length=seg_hop_length,
        ms_n_fft=ms_n_fft,
        ms_hop_length=ms_hop_length,
        ms_win_length=ms_win_length,
        ms_n_mels=ms_n_mels,
        ms_sr=ms_sr,
        ms_fmax=ms_fmax,
        ms_channel=ms_channel,
    ...
    model.eval()
    with torch.no_grad():
        y_hat = model(xb.to(dev), n_wins.to(dev)).cpu().numpy()
    return y_hat


def predict_dim_file(
    model: nn.Module,
    audio: AudioInput,
    dev: torch.device,
    *,
...
def is_const(x: FloatArray) -> bool:
...
def calc_eval_metrics(
    y: FloatArray,
    y_hat: FloatArray,
    y_hat_map: FloatArray | None = None,
    d: int | None = None,
    ci: FloatArray | None = None,
) -> MetricsDict:
    """
...
def calc_rmse(y_true: FloatArray, y_pred: FloatArray, d: int = 0) -> float:
    if d == 0:
...
def calc_mapped(x: FloatArray, b: FloatArray) -> FloatArray:
    N = x.shape[0]
    order = b.shape[0] - 1
...
def fit_first_order(y_con: FloatArray, y_con_hat: FloatArray) -> FloatArray:
    A = np.vstack([np.ones(len(y_con_hat)), y_con_hat]).T
    b = np.linalg.lstsq(A, y_con, rcond=None)[0]
    return b


def fit_second_order(y_con: FloatArray, y_con_hat: FloatArray) -> FloatArray:
    A = np.vstack([np.ones(len(y_con_hat)), y_con_hat, y_con_hat**2]).T
    b = np.linalg.lstsq(A, y_con, rcond=None)[0]
    return b


def fit_third_order(y_con: FloatArray, y_con_hat: FloatArray) -> FloatArray:
...
def fit_monotonic_third_order(
    dfile_db: pd.DataFrame,
    dcon_db: pd.DataFrame | None = None,
    pred: str | None = None,
    target_mos: str | None = None,
    target_ci: str | None = None,
...
def eval_results(
    df: pd.DataFrame,
    dcon: pd.DataFrame | None = None,
    target_mos: str = "mos",
    target_ci: str = "mos_ci",
    pred: str = "mos_pred",
    mapping: str | None = None,
    do_print: bool = False,
    do_plot: bool = False,
...
class biasLoss(object):
    ...
    def __init__(
        ...
        mapping: str | None = "first_order",
        min_r: float | None = 0.7,
    ...
    def get_loss(self, yb: torch.Tensor, yb_hat: torch.Tensor, idx: torch.Tensor | IntArray) -> torch.Tensor:
    ...
    def update_bias(self, y: FloatArray, y_hat: FloatArray) -> None:
    ...
    def _calc_bias_first_order(self, y_hat: FloatArray, y: FloatArray) -> FloatArray:
...
class earlyStopper(object):
    ...
    def step(self, r: MetricsDict) -> bool:
        self.best = False
...
# %% Dataset
class SpeechQualityDataset(Dataset[DatasetItem]):
    """
    ...
    def __init__(
        self,
        df: pd.DataFrame,
        df_con: pd.DataFrame | None = None,
        data_dir: str = "",
        folder_column: str = "",
        filename_column: str = "filename",
        mos_column: str = "MOS",
        seg_length: int | None = 15,
        max_length: int | None = None,
        to_memory: bool = False,
        to_memory_workers: int = 0,
        transform: Callable[[FloatArray], torch.Tensor | FloatArray] | None = None,
        seg_hop_length: int = 1,
        ms_n_fft: int = 1024,
        ms_hop_length: int = 80,
        ms_win_length: int = 170,
        ms_n_mels: int = 32,
        ms_sr: float = 48e3,
        ms_fmax: float = 16e3,
        ms_channel: int | None = None,
        double_ended: bool = False,
        filename_column_ref: str | None = None,
        dim: bool = False,
    ) -> None:
    ...
    def _to_memory_multi_helper(self, idx: list[int]) -> list[SpecData]:
    ...
    def _load_spec(self, index: int) -> SpecData:
        ...
        if self.seg_length is not None:
            ...
            if self.double_ended:
                x_spec_seg_ref, n_wins_ref = segment_specs(
                    spec_ref, self.seg_length, self.seg_hop_length, self.max_length
                )
        else:
            x_spec_seg = spec
            n_wins = spec.shape[1]
...
# %% Spectrograms
def segment_specs(
    x: torch.Tensor | FloatArray,
    seg_length: int,
    seg_hop: int = 1,
    max_length: int | None = None,
) -> tuple[torch.Tensor, IntArray]:
    """
    ...
    if n_wins < 1:
        raise ValueError(
            ...
            "Consider zero padding the audio sample."
        )
...
def get_librosa_melspec(
    ...
    """
    # Calc spec
    audio_source: str | Path | io.BytesIO
    ...
    try:
        if ms_channel is not None:
            y, sr = lb.load(audio_source, sr=sr, mono=False)
            if len(y.shape) > 1:
                y = y[ms_channel, :]
        else:
            y, sr = lb.load(audio_source, sr=sr)
    except Exception as exc:
        raise ValueError("Could not load audio input") from exc
